...
app.get("/recherche", (req, res) => {
  ...
  fs.readFile("all_data.json", "utf8", (err, data) => {
    ...
    const resultat = chaussures.filter((u) =>
      u.attributes.silhouette
        .toLowerCase()
...
app.post("/wishlist/:accountId", (req, res) => {
  ...
  const wishlistData = fs.readFileSync(`${accountId}.json`, "utf8");
...
app.delete("/wishlist/:accountId/sneakers/:sneakerId", (req, res) => {
  ...
  try {
    ...
    const indexToRemove = wishlistArray.findIndex(
      (sneaker) => String(sneaker.id) === sneakerId
    );
    ...
    if (indexToRemove !== -1) {
      ...
      fs.writeFileSync(`${accountId}.json`, JSON.stringify(wishlistArray));

      console.log(
        `Sneaker with ID ${sneakerId} removed from wishlist for account ${accountId}`
      );
      res
        .status(200)
        .json({ message: "Sneaker removed from wishlist successfully" });
    } else {
...
app.post("/login", (req, res) => {
  const username = req.body.username; // Récupère le terme de recherche de l'URL
  const password = req.body.password; // Récupère le terme de recherche de l'URL
  fs.readFile("login.json", "utf8", (err, data) => {
    ...
    // Filtrer les produits en fonction du terme de recherche
    const resultat = utilisateurs.filter(
      (u) =>
...
app.delete("/collection/:accountId/sneakers/:sneakerId", (req, res) => {
  ...
  try {
    ...
    // Trouver l'index de la sneaker dans la wishlist
    const indexToRemove = collectionArray.findIndex(
      (sneaker) => String(sneaker.id) === sneakerId
    );
    ...
    if (indexToRemove !== -1) {
      ...
      console.log(
        `Sneaker with ID ${sneakerId} removed from collection for account ${accountId}`
      );
      res
        .status(200)
        .json({ message: "Sneaker removed from collection successfully" });
    } else {
